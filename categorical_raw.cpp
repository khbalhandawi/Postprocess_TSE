...
int main ( int argc , char ** argv )
{
	...
	try
	{
		...
		xt.display_eval(cout);
		NOMAD::Point fout = xt.get_bb_outputs();
		cout << fout << endl;
		cout << "nomad_begin" << endl;

		// extended poll:
		My_Extended_Poll ep ( p );
		
		ep.construct_extended_points(xt);
...
_s3           ( NULL )
{
	...
	// -----------------------
	{
		...
		for ( int i = 0 ; i < 5 ; ++i )
		{
			//i = 0; 1 + -1%2 = 0 categorical
			//i = 1; 1 + 0%2 = 1 categorical
			//i = 2; 1 + 1%2 = 2 continous
			//i = 3; 1 + 2%2 = 1 categorical
			//i = 4; 1 + 3%2 = 2 continous
...
void My_Extended_Poll::construct_extended_points ( const Eval_Point & x )
{
	...
	else if ( n == 2 )
	{
		...
		// change the type of one asset (2 neighbors):
		{
			...
			extended.push_back(y1);
			extended.push_back(y2);
		}
